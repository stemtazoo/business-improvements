...
def ensure_schema(con: sqlite3.Connection) -> None:
    """
    Create tables if not exist. Minimal set to start accumulating facts.
    (contexts/units later)
    """
    con.execute(
        """
    ...
    con.execute(
        ...
        CREATE TABLE IF NOT EXISTS facts (
          ...
          UNIQUE (
            filing_id, name, context_ref, unit_ref, value_text, source_file
          )
        );
        """
    )
    ...
    # Helpful indexes for downstream apps
